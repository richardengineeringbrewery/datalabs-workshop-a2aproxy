...
The agent card served at /.well-known/agent-card.json comes from the
agent-card.json file bundled into this image at build time (Elastic Agent
Builder has no live HTTP endpoint to fetch it from) — its security scheme is
replaced with ours before serving.
"""
...
def _load_agent_card_template():
    # Elastic's agent card export uses triple-quoted strings for multi-line
    # description fields, which isn't valid JSON — repair before parsing.
    raw = open("agent-card.json", encoding="utf-8").read()
    fixed = re.sub(r'"""(.*?)"""', lambda m: json.dumps(m.group(1)), raw, flags=re.DOTALL)
    return json.loads(fixed)


try:
    AGENT_CARD_TEMPLATE = _load_agent_card_template()
except (OSError, json.JSONDecodeError) as exc:
    sys.exit(f"failed to load agent-card.json: {exc}")
